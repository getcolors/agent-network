...
for h in agent-network.example.com $ep; do
  exp=$(echo | openssl s_client -servername "$h" -connect "agent-network.example.com:443" 2>/dev/null \
...
if [[ -s /etc/agent-network/secrets/pat ]]; then
  curl -fsS -H "Authorization: Token $(cat /etc/agent-network/secrets/pat)" \
    "https://agent-network.example.com/api/agent-network/access-logs?page=1&page_size=1" 2>/dev/null \
...
echo; echo "== dashboard"
echo "  https://agent-network.example.com/  (admin: admin@example.com;"
