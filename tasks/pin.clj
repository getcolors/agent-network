...
(def path "skills/package-agent-network-green/green")
(def rx #"\(def \^:private agent-network-sha (nil|\"[0-9a-f]{40}\")\)")
...
(let [dirty (git "status" "--porcelain") sha (git "rev-parse" "HEAD") remotes (git "branch" "-r" "--contains" sha)]
  (cond (seq dirty) (do (binding [*out* *err*] (println "agent-network working tree is dirty; commit before pinning")) (System/exit 2))
        ...
        :else (let [s (slurp path) n (str/replace s rx (str "(def ^:private agent-network-sha \"" sha "\")"))]
                (spit path n) (println "pinned agent-network launcher to" (subs sha 0 7)))))
