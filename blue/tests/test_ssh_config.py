...
from package_agent_network_blue import ssh_config, tools, workflow

# §2 the alias and the identity file


def test_alias_is_the_profile():
    assert ssh_config.host_alias(fixture()) == "agent-network-fixture"
...
def test_identity_file_keeps_the_tilde(tmp_path, monkeypatch):
    ...
    assert ssh_config.identity_file(fixture()) == "~/.ssh/agent-network-fixture"
...
def test_the_marker_is_the_alias_alone():
    ...
    # too would repeat it: "# BEGIN agent-network agent-network-vultr".
    assert ssh_config.begin_marker("agent-network-vultr") == "# BEGIN agent-network-vultr ANSIBLE MANAGED BLOCK"
    assert ssh_config.end_marker("agent-network-vultr") == "# END agent-network-vultr ANSIBLE MANAGED BLOCK"
...
def test_a_foreign_stanza_is_found():
    lines = ["Host other", "    HostName 192.0.2.1", "", "Host agent-network-fixture"]
    assert ssh_config.foreign_stanza_line(lines, "agent-network-fixture") == 4


def test_our_own_block_is_not_foreign():
    alias = "agent-network-fixture"
...
def test_a_block_under_a_retired_marker_is_foreign():
    ...
    alias = "agent-network-vultr"
    lines = [f"# BEGIN agent-network {alias} ANSIBLE MANAGED BLOCK",
             f"Host {alias}",
             f"# END agent-network {alias} ANSIBLE MANAGED BLOCK"]
    assert ssh_config.foreign_stanza_line(lines, alias) == 2


def test_a_multi_pattern_host_line_counts():
    assert ssh_config.foreign_stanza_line(["Host web agent-network-fixture db"], "agent-network-fixture") == 1


def test_an_unrelated_file_is_left_alone():
    assert ssh_config.foreign_stanza_line(["Host build", "Host agent-network-other"],
                                          "agent-network-fixture") is None
...
def test_the_local_play_renders_no_address():
    ...
    assert data["ssh-config-identity-file"] == "~/.ssh/agent-network-fixture"
...
def test_the_local_stage_renders_three_files():
    ...
    assert all("agent-network-ansible-local" in t for t in targets)
...
def test_delete_removes_the_block_before_the_destroy():
    ...
    assert workflow.wire_fn("agent-network/dns", delete)[1:] == ("agent-network/ssh-config",)
    assert workflow.wire_fn("agent-network/ssh-config", delete)[1:] == ("agent-network/infrastructure",)
