import pytest
from package_agent_network_blue import tools
...
async def test_retired_delete_ignores_stale_host(monkeypatch,tmp_path,event,retired,expected):
    ...
    result=await tools.ansible_step({'profile':'test','workdir':str(tmp_path),'blue/event':event,'agent-network/already-destroyed':retired,'ip':'203.0.113.19','ssh-private-key-path':str(tmp_path/'removed-key')})
