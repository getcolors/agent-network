...
def wire_fn(step: str, run_opts: dict):
    if run_opts.get("blue/event") == "delete":
        return {
            "agent-network/start": (start_step, "agent-network/load"), "agent-network/load": (compute.load_step, "agent-network/ansible"),
            ...
            "agent-network/infrastructure": (tools.infrastructure_step,),
        }.get(step)
    return {
...
side_effecting = ["agent-network/infrastructure", "agent-network/dns", "agent-network/ssh-config",
                  "agent-network/ansible", "agent-network/acceptance", "agent-network/load"]


def create_workflow():
    wf = workflow(start="agent-network/start", wire_fn=wire_fn)
    wf = advice_add(wf, "agent-network/dns", "before", "agent-network.workflow/backend", backend_advice(tools.dns_tool))
