"""Singleton compute delegation; DNS and application configuration remain local."""
import json
...
def requirements(opts):
    ssh = source_cidrs(opts, 'ssh-sources', 'agent-network-ssh-sources')
    http = source_cidrs(opts, 'http-sources', 'agent-network-http-sources')
    if not ssh: raise ValueError('SSH source list must not be empty')
    stun = source_cidrs(opts, 'stun-sources', 'agent-network-stun-sources')
    return {'single_host': True, 'ipv6':False, 'security': {'egress': 'all', 'private_filter': False, 'ingress': [
        ...
        *([{'id':'stun','protocol':'udp','from_port':opts['agent-network-stun-port'],'to_port':opts['agent-network-stun-port'],'sources':stun}] if stun else [])
    ]}, 'legacy_state_keys': [opts['profile'] + '/agent-network-infrastructure.tfstate']}
...
def attach(opts, result):
    ...
    if result.get('status') == 'destroyed':
        return {**opts, 'blue/exit': 0, 'agent-network/already-destroyed': True}
