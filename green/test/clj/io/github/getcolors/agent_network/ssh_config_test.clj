(ns io.github.getcolors.agent-network.ssh-config-test
...
(deftest alias-is-the-profile
  (is (= "agent-network-fixture" (ssh-config/host-alias (fixture)))))
...
(deftest identity-file-keeps-the-tilde
  ...
  (is (= "~/.ssh/agent-network-fixture" (ssh-config/identity-file (fixture))))
...
(deftest the-marker-is-the-alias-alone
  ...
  (is (= "# BEGIN agent-network-vultr ANSIBLE MANAGED BLOCK"
         (ssh-config/begin-marker "agent-network-vultr")))
  (is (= "# END agent-network-vultr ANSIBLE MANAGED BLOCK"
         (ssh-config/end-marker "agent-network-vultr"))))

;; §5 never adopt

(deftest a-foreign-stanza-is-found
  (let [lines ["Host other" "    HostName 192.0.2.1" "" "Host agent-network-fixture"]]
    (is (= 4 (ssh-config/foreign-stanza-line lines "agent-network-fixture")))))

(deftest our-own-block-is-not-foreign
  (let [alias "agent-network-fixture"
...
(deftest a-block-under-a-retired-marker-is-foreign
  ;; The superseded `# BEGIN agent-network <alias>` marker is gone, so a block
  ...
  (let [alias "agent-network-vultr"
        lines [(str "# BEGIN agent-network " alias " ANSIBLE MANAGED BLOCK")
               (str "Host " alias)
               (str "# END agent-network " alias " ANSIBLE MANAGED BLOCK")]]
    (is (= 2 (ssh-config/foreign-stanza-line lines alias)))))

(deftest a-multi-pattern-host-line-counts
  (is (= 1 (ssh-config/foreign-stanza-line ["Host web agent-network-fixture db"]
                                           "agent-network-fixture"))))

(deftest an-unrelated-file-is-left-alone
  (is (nil? (ssh-config/foreign-stanza-line ["Host build" "Host agent-network-other"]
                                            "agent-network-fixture"))))
...
(deftest the-local-play-renders-no-address
  ...
  (let [data (tools/ansible-local-data (assoc (fixture) :ip "203.0.113.7"))]
    ...
    (is (= "~/.ssh/agent-network-fixture" (:ssh-config-identity-file data)))))
...
(deftest the-local-stage-renders-three-files
  (let [targets (map #(str (:target %)) (tools/ansible-local-specs (fixture)))]
    ...
    (is (every? #(str/includes? % "agent-network-ansible-local") targets))))
