(ns io.github.getcolors.agent-network.compute
...
(defn requirements [opts]
 (let [ssh (deployment/source-cidrs opts "ssh-sources" "agent-network-ssh-sources") http (deployment/source-cidrs opts "http-sources" "agent-network-http-sources") stun (deployment/source-cidrs opts "stun-sources" "agent-network-stun-sources")]
  ...
  {:single_host true :ipv6 false :security {:egress "all" :private_filter false
   :ingress (into [{:id "ssh" :protocol "tcp" :from_port 22 :to_port 22 :sources ssh}
             {:id "stun" :protocol "udp" :from_port (:agent-network-stun-port opts) :to_port (:agent-network-stun-port opts) :sources stun}]
                  (map (fn [port] {:id (str "web-" port) :protocol "tcp" :from_port port :to_port port :sources http}) [80 443]))}
   :legacy_state_keys [(str (:profile opts) "/agent-network-infrastructure.tfstate")]}))
...
(defn attach [opts result]
 (cond
  ...
  (= "destroyed" (:status result)) (assoc opts :green/exit 0 :agent-network/already-destroyed true)
