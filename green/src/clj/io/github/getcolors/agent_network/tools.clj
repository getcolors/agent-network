...
(defn dns-step [opts]
  (if (:agent-network/already-destroyed opts) opts
...
(defn ansible-step [opts]
  (let [dir (tool-dir opts ansible-tool)]
    (if (and (= :delete (:green/event opts)) (or (:agent-network/already-destroyed opts) (not (:ip opts))))
...
(defn ssh-out
  ...
  (let [args (concat ["ssh" "-o" "BatchMode=yes" "-o" "StrictHostKeyChecking=accept-new"]
                     ...
                     [(str (or (:user opts) "root") "@" (:ip opts)) command])]
    (out (vec args))))
