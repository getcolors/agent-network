...
if ! in_agent timeout 5 bash -c "</dev/tcp/<{ traefik-agent-ip }>/443" 2>/dev/null; then
...
for target in "<{ traefik-agent-ip }>/443" "<{ traefik-agent-ip }>/80" "<{ proxy-agent-ip }>/8443"; do
  host=${target%/*}; port=${target#*/}
...
if [[ -z $peer_id ]]; then
  log "FAIL: no recorded agent peer id; run the post-enroll step before smoke"
  exit 1
fi
